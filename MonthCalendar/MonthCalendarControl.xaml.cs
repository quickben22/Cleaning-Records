...
namespace MonthCalendar
{
    ...
    public partial class MonthCalendarControl : UserControl
    {
        ...
        List<SolidColorBrush> brushes = new List<SolidColorBrush>{Brushes.Red,Brushes.Black,Brushes.Blue,Brushes.Brown,
            Brushes.Violet,Brushes.Pink,Brushes.NavajoWhite,Brushes.Azure,Brushes.Aquamarine,Brushes.DarkGoldenrod,Brushes.DeepPink };
        private void BuildCalendarUI()
        {
            int brushesCount = brushes.Count;
            int iDaysInMonth = sysCal.GetDaysInMonth(_DisplayStartDate.Year, _DisplayStartDate.Month);
            int iOffsetDays = (Convert.ToInt32(System.Enum.ToObject(typeof(System.DayOfWeek), _DisplayStartDate.DayOfWeek)) + 6) % 7;
            int iWeekCount = 0;
            ...
            for (int i = 1; i <= iDaysInMonth; i++)
            {
                ...
                dayBox.DayNumberLabel.Content = i.ToString("D2") + $".{_DisplayMonth.ToString("D2")}.{_DisplayYear}.";
                dayBox.Tag = i;
                ...
                else if (_monthAppointments != null)
                {
                    ...
                    int iday = i;
                    List<Appointment> aptInDay = _monthAppointments.FindAll(new System.Predicate<Appointment>((Appointment apt) => Convert.ToDateTime(apt.Date).Day == iday));
                    foreach (Appointment a in aptInDay)
                    {
                        ...
                        if (a.CleanerID != null)
                        {
                            if (a.Color != null)
                            {
                                try
                                {
                                    apt.DisplayText.Foreground = (SolidColorBrush)(new BrushConverter().ConvertFrom(a.Color));
                                }
                                catch
                                {
                                ...
                                apt.DisplayText.Foreground = brushes[(int)a.CleanerID % brushesCount];
                        }
